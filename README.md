...
4. 批量下载时，同一个服务器文件，只能下载到本地的一个目录中，如果一个服务器文件，需要上传到本地的不同目录中，则只会读取最后一个配置项（因为用map匹配）

5. 批量上传时，配置文件ftpBatchUpload.txt中本地目录和服务器目录之间需要保持4个table的空格,否则会读取异常，结果会有影响

6. 文件与目录的认定标准：文件名中有“.”而目录没有
